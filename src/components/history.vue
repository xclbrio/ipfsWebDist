    ...
    <vue-tabs>
      <v-tab title="TRADE HISTORY">
        <div class="history__table">
          ...
          <div class="history__container">
            <a
              target="_blank"
              :href="`${settings.network.etherscan}tx/${item.txHash}`"
              v-for="item in historyData"
              :class="item.orderType"
              class="row history__row"
            >
      ...
      <v-tab title="PERSONAL TH">
        <div class="history__table">
          ...
          <div class="history__container">
            <a
              :href="`https://kovan.etherscan.io/tx/${item.txHash}`"
              v-for="item in personalHistoryData"
              :class="item.orderType"
              class="row history__row"
            >
              <div class="col amount">
...
export default {
    ...
    personalHistoryData() {
      if (this.from !== null && this.from !== undefined) {
        return this.historyData.filter(
          (element) =>
            element.maker.toLowerCase() == this.from.toLowerCase() ||
            element.taker.toLowerCase() == this.from.toLowerCase()
        );
      }
    },
    tokenGetAddress() {
  ...
  methods: {
    formating(el) {
      ...
      el.formatDate = `${
        el.formatDate.getUTCMonth() + 1 < 10
          ? "0" + (el.formatDate.getUTCMonth() + 1)
          : el.formatDate.getUTCMonth() + 1
      }/${
        el.formatDate.getUTCDate() < 10
          ? "0" + el.formatDate.getUTCDate()
          : el.formatDate.getUTCDate()
      } ${
        el.formatDate.getUTCHours() < 10
          ? "0" + el.formatDate.getUTCHours()
          : el.formatDate.getUTCHours()
      }:${
        el.formatDate.getUTCMinutes() < 10
          ? "0" + el.formatDate.getUTCMinutes()
          : el.formatDate.getUTCMinutes()
      }`;
